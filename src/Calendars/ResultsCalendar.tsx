...
const ResultsCalendar: React.FC<ResultsCalendarProps> = ({
    ...
    const filteredTable = useMemo<Lesson[]>(
        () => (showOwnSubjects ? [...lessonsResults, ...ownLessons.filter(lesson => !lessonsResults.some(lessonRes => lessonRes.id === lesson.id))] : lessonsResults),
    ...
    return (
        <LessonCalendar 
            ...
            popoverActionIcon={(id:number) => isEventInSaved(id) ? <EventBusyIcon fontSize='small'/> : <EventAvailableIcon fontSize='small'/>}
            popoverActionText={(id:number) => isEventInSaved(id) ? "Kattints az órarendből törléshez" : "Kattints az órarendbe adáshoz"}
            >
                <Button
                        variant="outlined"
